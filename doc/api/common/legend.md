...
<p class='ev_expand_introduce'>说明：图例图标的宽度，<code>legend.icon</code> 不为 <code>circle</code> 时，此默认值为 12
...
<p class='ev_expand_introduce'>说明：图例图标的富文本配置，<code>theme=light</code>，color='#777777'，<code>theme=dark</code>，color='#C9C9C9'。<code>width</code>设置每个图例文本的宽度，<code>overflow</code>设置图例文本过长后的显示状态，可选：none：不设置、truncate：截断且显示省略号、break：中文换行、breakAll：强制单词换行。
