...
function calculateOccupancy(iChartOption, legend, legendData){
  try {
    ...
    for (const key in richMaxWidth) {
      ...
      // if(key === 'split') element.maxWidth = richStyles[key].width
    }
console.log(11112)
    // 计算结果
    const maxWidth = Math.max(...itemWidths) + 6; //icon与文本的间隙
    //清理容器
...
function updateLegendOccupancy(iChartOption, legend, legendData, chartInstance){
  ...
  const {richMaxWidth, maxWidth, titleName, hasSplit} = config;
  // 用户formatter
  const iLegendFormatter = formatter;
  // 用户rich
  const rich = cloneDeep(textStyle.rich);
  let titleExceed = false;
  let titleWidth = 0;
  let splitWidth = hasSplit ? 10 : 0
  const iTitleMaxWidth = Number(rich?.[titleName]?.width) || 0;
  const titleMaxWidth = richMaxWidth?.[titleName]?.maxWidth || 0;
  if (legendMaxWidth < maxWidth) {
    let notTitleMaxWidth = 0; //除去title的占宽
    
    titleExceed = true;
    for (const key in richMaxWidth) {
      if (key !== titleName){
        const element = richMaxWidth[key];
        const width = rich?.[key]?.width ? Number(rich?.[key]?.width) : element.maxWidth;
        notTitleMaxWidth += width;
      }
    }
    titleWidth = titleMaxWidth - (maxWidth - legendMaxWidth) - 16 - splitWidth;
    titleWidth = iTitleMaxWidth ? iTitleMaxWidth : titleWidth;
    legend.tooltip = { show: true};
  } else {
    titleWidth = iTitleMaxWidth ? iTitleMaxWidth : titleMaxWidth//(titleMaxWidth - 16 - splitWidth);
  }
