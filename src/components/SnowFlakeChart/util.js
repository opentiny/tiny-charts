...
const getCssNumber = (dom, cssName) => {
...
// 获取两个dom的距离
const getDistance = (dom1, dom2) => {
  const { x: x1, y: y1, width: width1, height: height1 } = dom1.getBoundingClientRect();
  const { x: x2, y: y2, width: width2, height: height2 } = dom2.getBoundingClientRect();
  let center1 = {
    x: x1 + width1 / 2,
    y: y1 + height1 / 2
  };
...
// 悬浮提示框的边界判断
const boundaryJudge = (e, container, toolTip) => {
  const distance = 20;
  let x = e.clientX + distance;
  let y = e.clientY + distance;
  const { width: containerWidth, height: containerHeight, x: containerLeft, y: containerTop } = container.getBoundingClientRect();
  const { width: toolTipWidth, height: toolTipHeight } = toolTip.getBoundingClientRect();
  if (x + toolTipWidth > containerLeft + containerWidth) { // 超过右边界
    x = e.clientX - toolTipWidth - distance / 2;
  }
  if (y + toolTipHeight > containerTop + containerHeight) { // 超过下边界
