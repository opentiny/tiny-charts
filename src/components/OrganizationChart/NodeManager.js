...
export default class NodeManager {
    ...
    createWarppers() {
        ...
        for (let index = 0; index < this.depth; index++) {
            ...
            let inertia = this.autoRotate?.inertia;
            // 圆环
            let warpper = `<div class="ozc_warpper ${inertia === false ? 'ozc_no_inertia': ''}" style='width: ${width}px;height: ${width}px;z-index: ${zindex}; transform: translateX(-50%) translateY(-50%) rotate(0deg);'></div>`;
    ...
    createNode(data, style, warpperIndex, nodeIndex) {
        ...
        let inertia = this.autoRotate?.inertia;
        let nodeClass = inertia === false ? 'ozc_card ozc_no_inertia' : 'ozc_card';
