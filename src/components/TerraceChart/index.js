...
import { isString, isDOM, isArray, isNumber } from '../../util/type';
...
export default class TerraceChart extends BaseChart {
    ...
    static name = CHART_TYPE.TERRACE
    ...
    getNewRadius(radius) {
        if (isNumber(radius)) {
            const outerRadius = radius / (Math.min(this.container.getBoundingClientRect().width, this.container.getBoundingClientRect().height) / 2) * 100;
            return ['30%', outerRadius];
        } else if (isString(radius)) {
            const outerRadius = radius.endsWith('%') ? radius : (parseFloat(radius) / (Math.min(this.container.getBoundingClientRect().width, this.container.getBoundingClientRect().height) / 2) * 100);
            return ['30%', outerRadius];
        } else if (isArray(radius)) {
            const radiusArr = radius.map(r => isNumber(r) ? r / (Math.min(this.container.getBoundingClientRect().width, this.container.getBoundingClientRect().height) / 2) * 100
                : r.endsWith('%') ? r : (parseFloat(r) / (Math.min(this.container.getBoundingClientRect().width, this.container.getBoundingClientRect().height) / 2) * 100));
            return radiusArr;
        }
    }
