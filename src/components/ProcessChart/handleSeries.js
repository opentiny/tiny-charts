...
function handleBasicProcessTheme(baseOpt, colorGroup, doubleSide) {
  if (doubleSide) {
    ...
    baseOpt.series[0].label.color = colorGroup.axislabel;
    baseOpt.series[1].label.color = colorGroup.axislabel;
    baseOpt.series[1].itemStyle.color = colorGroup.subg;
  }
}

function handleStackProcessTheme(baseOpt, iChartOpt, colorGroup) {
  // 背景柱条颜色
  baseOpt.series[1].itemStyle.color = colorGroup.subg;
  // 右侧label颜色
  baseOpt.series[1].label.color = colorGroup.axislabel;
  ...
  baseOpt.series.forEach((serie, index) => {
    if (index > 1) {
      ...
      serie.label.color = colorGroup.font;
    }
  });
}
...
function handleBasicProcessLabel(baseOpt, iChartOpt, dataSet, doubleSide) {
  if (doubleSide) {
    ...
    const valWidth_l = getMaxValLength(dataSet, iChartOpt, 0);
    const valWidth_r = getMaxValLength(dataSet, iChartOpt, 1);
...
function handleTheme(baseOpt, iChartOpt, doubleSide) {
  ...
  if (iChartOpt.name === CHARTTYPENAME.ProcessBarChart) {
    handleBasicProcessTheme(baseOpt, colorGroup, doubleSide);
  } else {
    handleStackProcessTheme(baseOpt, iChartOpt, colorGroup);
  }
}
