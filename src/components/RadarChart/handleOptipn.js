...
function handleRader(iChartOption, indicator, dataLength, data) {
  ...
  // 阈值告警色
  const alarmColor = Theme.color.state.error;
  const axisLine = handleAxisLine(colorBase);
  const axisLabel = handleAxisLabel(iChartOption.radarMark);
  const splitLine = handleSplitLine(colorBase);
  const rich = handleRich(alarmColor, colorBase);
