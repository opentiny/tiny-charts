...
function group() {
    ...
    this.item.forEach((item) => {
        ...
        let pre = Math.floor((getOffsetLeft(item) - barLeft + windowScrollX) / this.scroll.clientWidth);
        let cur = Math.floor((getOffsetLeft(item) - barLeft + windowScrollX + item.clientWidth) / this.scroll.clientWidth);
        if (pre + 1 === cur) {
    ...
    if(this.group.length>0){
        let count = this.group[0].length-1
